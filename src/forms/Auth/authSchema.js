...
const {
  ...
  register: {
    formField: { username, registerEmail, registerPassword },
  },
} = authModel;
...
export default {
  ...
  registerSchema: Yup.object().shape({
    [username.name]: Yup.string()
      ...
      .label(username.name),
    [registerEmail.name]: Yup.string()
      .required(registerEmail.error.required)
      .label(registerEmail.name),
    [registerPassword.name]: Yup.string()
      .required(registerPassword.error.required)
